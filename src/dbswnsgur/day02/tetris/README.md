...
MySQL과 앱 컨테이너가 함께 시작된다. MySQL 헬스체크가 통과된 뒤 앱이 자동으로 시작된다.
...
MySQL 없이도 인메모리 SQLite로 실행 가능하다.
...
- **DB 격리**: `sqlite:///:memory:` + `StaticPool`으로 실제 DB를 건드리지 않음. `reset_db` 픽스처(autouse)가 각 테스트 전후로 테이블을 생성/삭제해 완전히 격리됨.
...
- **백엔드**: FastAPI, SQLAlchemy, MySQL 8.0, python-jose, passlib
- **프론트엔드**: 바닐라 JS, HTML5 Canvas
- **인프라**: Docker Compose
- **테스트**: pytest, httpx (인메모리 SQLite)

## 알려진 이슈

- `passlib 1.7.4`는 `bcrypt 4.0+`와 호환되지 않는다 (`detect_wrap_bug` 내부에서 ValueError 발생 → 회원가입/로그인 500 에러). `requirements.txt`에 `bcrypt<4.0.0`으로 버전을 고정해 해결한다.
